...
import 'package:poc/widgets/calender_picker/event.dart';
import 'package:poc/widgets/calender_picker/event_list.dart';
import 'package:poc/widgets/calender_picker/header.dart';
import 'package:poc/widgets/calender_picker/default_style.dart';
import 'package:poc/widgets/calender_picker/multiple_marked_dates.dart';
import 'package:poc/widgets/calender_picker/weekday_row.dart';
...
/// [isSelectable] - is between [PrimaryCalendar.minSelectedDate] and [PrimaryCalendar.maxSelectedDate]
...
/// Example: if the user provided [PrimaryCalendar.todayTextStyle] and [isToday] is true,
///   [PrimaryCalendar.todayTextStyle] would be sent into [DayBuilder]'s [textStyle]. If user didn't
///   provide it, default [PrimaryCalendar]'s textStyle would be sent. Same applies to all text styles like
///   [PrimaryCalendar.prevDaysTextStyle], [PrimaryCalendar.daysTextStyle] etc.
/// [isNextMonthDay] - if the day is from next month
/// [isThisMonthDay] - if the day is from next month
/// [day] - day being built.
typedef DayBuilder = Widget? Function(bool isSelectable, int index, bool isSelectedDay, bool isToday, bool isPrevMonthDay,
...
class PrimaryCalendar<T extends EventInterface> extends StatefulWidget {
...
class CalendarState<T extends EventInterface> extends State<PrimaryCalendar<T>> {
  ...
  @override
  void didUpdateWidget(PrimaryCalendar<T> oldWidget) {
        ...
        children: <Widget>[
          CalendarHeader(
            ...
            headerTitle: headerText ??
                (widget.weekFormat ? _localeDate.format(_weeks[_pageNum].first) : _localeDate.format(_dates[_pageNum])),
            ...
            onLeftButtonPressed: () {
              ...
              if (_pageNum > 0) _setDate(_pageNum - 1);
            },
            onRightButtonPressed: () {
              widget.onRightArrowPressed?.call();

              if (widget.weekFormat) {
                if (_weeks.length - 1 > _pageNum) _setDate(_pageNum + 1);
              } else {
                if (_dates.length - 1 > _pageNum) _setDate(_pageNum + 1);
              }
            },
    ...
    return Center(
      child: SizedBox(
        ...
        child: Row(
          ...
          children: <Widget>[
            DefaultTextStyle(
              style: getDefaultDayStyle(isSelectable, index, isSelectedDay, isToday, isPrevMonthDay, textStyle, defaultTextStyle,
                  isNextMonthDay, isThisMonthDay),
              child: Text(
                '${now.day}',
                semanticsLabel: now.day.toString(),
                style: getDayStyle(isSelectable, index, isSelectedDay, isToday, isPrevMonthDay, textStyle, defaultTextStyle,
      ...
      child: GestureDetector(
        ...
        child: TextButton(
          style: TextButton.styleFrom(
            ...
            // shape:
            //     widget.markedDateCustomShapeBorder != null && markedDatesMap != null && markedDatesMap.getEvents(now).isNotEmpty
          ...
          child: Stack(
            children: widget.showIconBehindDayText
                ? <Widget>[
                    ...
                    getDayContainer(isSelectable, index, isSelectedDay, isToday, isPrevMonthDay, textStyle, defaultTextStyle,
                        isNextMonthDay, isThisMonthDay, now),
                  ]
                : <Widget>[
                    getDayContainer(isSelectable, index, isSelectedDay, isToday, isPrevMonthDay, textStyle, defaultTextStyle,
            ...
            child: SizedBox(
              ...
              child: GridView.count(
                ...
                children: List.generate(totalItemCount,
                  ...
                  return renderDay(isSelectable, index, isSelectedDay, isToday, isPrevMonthDay, textStyle, defaultTextStyle,
                      isNextMonthDay, isThisMonthDay, now);
                }),
              ),
            ),
              ...
              child: SizedBox(
                ...
                child: GridView.count(
                  ...
                  children: List.generate(weekDays.length, (index) {
                    ...
                    bool isSelectedDay = _selectedDate.year == weekDays[index].year &&
  ...
  void _setDatesAndWeeks() {
    ...
    for (int _cnt = 0;
        0 >= minDate.add(Duration(days: 7 * _cnt)).difference(maxDate.add(const Duration(days: 7))).inDays;
        _cnt++) {
    ...
    if (page == -1) {
      ...
      if (onCalendarChanged != null) {
        WidgetsBinding.instance.addPostFrameCallback((_) {
          onCalendarChanged(!widget.weekFormat ? _dates[page] : _weeks[page][firstDayOfWeek]);
        });
      }
    }
  }
                  ...
                  child: Center(
                    child: Text(
                      ...
                      style: markedDateMoreCustomTextStyle ??
                          const TextStyle(fontSize: 9.0, color: Colors.white, fontWeight: FontWeight.normal),
                    ),
                  ),
                ),
              ),
  ...
  Widget getDayContainer(bool isSelectable, int index, bool isSelectedDay, bool isToday, bool isPrevMonthDay,
    ...
    if (customDayBuilder != null) {
      ...
      dayContainer = customDayBuilder(
          isSelectable, index, isSelectedDay, isToday, isPrevMonthDay, styleForBuilder, isNextMonthDay, isThisMonthDay, now);
    }
