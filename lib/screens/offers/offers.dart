...
import 'package:poc/screens/offers/data/provider/offer_provider.dart';
...
class CartOffers extends ConsumerStatefulWidget{
  const CartOffers({Key? key}) : super(key: key);
  @override
  ConsumerState<CartOffers> createState() => _CartOffersState();
}
class _CartOffersState extends ConsumerState<CartOffers> {
  @override
  void initState() {
    super.initState();
    WidgetsBinding.instance.addPostFrameCallback((timeStamp) {
      ref.read(offerProvider).initState(context);
      print(ref.read(offerProvider).offerList);
    });
  }
  List<Widget> widgetList=<Widget>[];
  @override
  Widget build(BuildContext context) {
    final rProvider = ref.read(offerProvider);
    final wProvider = ref.watch(offerProvider);
    ...
    return Drawer(
      ...
      child:Stack(
        children: [
                      ...
                      children: [
                        ...
                        Container(
                          child: ListView.builder(
                              ...
                              itemCount: wProvider.offerList?.length??0,
                              itemBuilder: (_
                                  , index) {

                                final element=wProvider.offerList?[index];
                                print(element);
                                return Column(
                                  children: [
                                    Container(
                                        ...
                                        children: [
                                          Row(
                                            ...
                                            children: [
                                              ...
                                              InkWell(
                                                onTap: () {
                                                  rProvider.applyOffer(element?.promoCodeName.toString().toUpperCase()??'N/A', context);
                                                },
                                          ...
                                          Row(
                                            ...
                                            children: [
                                              SizedBox.square(
                                                dimension: 100,
                                                child: Image.network( element?.thumbImg ?? ''),
                                              ),
                                              const SizedBox(
                                                width: 10,
                                              ),
          ...
          if(rProvider.loading)const PrimaryLoader(),
        ],
      )
